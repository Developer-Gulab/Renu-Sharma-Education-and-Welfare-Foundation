...
const Impact = () => {
  ...
  return (
    <div className="">
      <h2 className="text-6xl md:text-7xl font-bold text-center pt-20 text-white">
        Our Impact
      </h2>
      <motion.div className="flex flex-col md:flex-row  justify-center gap-8 px-10 md:mt-12">
        {imapctStats.map((stat, index) => (
          <motion.div
            ...
            className="flex flex-col items-center gap-2 px-8 md:px-12 py-8 bg-white text-center rounded-3xl font-bold  md:block cursor-pointer text-white bg-opacity-20"
          >
            <AnimatedNumber value={stat.stat} className="text-6xl font-bold" />
            <h4 className="text-xl md:text-3xl font-semibold  md:mt-4">
              {stat.title}
            </h4>
            <p className="text-sm mt-2 font-normal">{stat.description}</p>
          </motion.div>
        ))}
      </motion.div>
    </div>
