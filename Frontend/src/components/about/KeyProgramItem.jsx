...
const KeyProgramItem = ({
  ...
  return (
    <div
      className="bg-white p-4 rounded-lg shadow-lg transition-all duration-300 hover:scale-105 flex flex-col items-center w-full"
      data-aos={aosAnimation}
      data-aos-delay={aosDelay}
    >
      <div className="text-3xl text-[#001F3F] mb-2">{icon}</div>
      <h3 className="text-lg font-semibold mb-2 text-[#001F3F] text-center">
        {title}
      </h3>
      <p className="text-sm mb-2 text-center text-gray-700">{description}</p>
      <div className="flex justify-around w-full">
        {stats.map((stat, index) => (
          <div key={index} className="text-center">
            ...
            <span className="text-gray-700 block text-sm">{stat.label}</span>
          </div>
        ))}
      </div>
    </div>
